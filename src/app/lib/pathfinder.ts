...
export const sign = n => (n == 0 ? 0 : Math.round(n / Math.abs(n)));
...
export const findPath = ({
  ...
  const closedSet = [];
  const openSet = [{ x: characterX, y: characterY, cost: 0, cameFrom: undefined }];

  const exploredNodes = exploreNodes(openSet, closedSet, arena);
  const targetNode = exploredNodes.find(node => node.x === targetX && node.y === targetY);
...
const tracePath = (node, path): Point[] =>
  node
    ? tracePath(node.cameFrom, path.concat({ x: node.x, y: node.y }))
    : path;
...
const nodeNeighbours = (node: PathNode, arena: Arena): PathNode[] =>
  [-1, 0, 1]
    ...
    .map(({ x, y, terrain, offsetX, offsetY }) => {
      ...
      const additionalCost = offsetX !== 0 && offsetY !== 0
        ? cost * Math.sqrt(2)
        : cost;
